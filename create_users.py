...
def create_users(n):
    ...
    my_id = CustomUser.objects.get(username="OnoTeruaki").id
    ...
    talks = Message.objects.order_by("-created_date")[: 2 * len(user_ids)]
    for talk in talks:
        talk.time = fakegen.date_time_this_year(tzinfo=tz.gettz("Asia/Tokyo"))
    Message.objects.bulk_update(talks, fields=["created_date"])
